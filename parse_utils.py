...
def created_name_tuple_class(fname,class_name):
...
def iter_file(fname,class_name,parser):
    nt_class = created_name_tuple_class(fname,class_name)
    reader = csv_parser(fname)
    for row in reader:
        parsed_data = (parse_fn(value)for value,parse_fn in zip(row,parser))
        yield nt_class(*parsed_data) #yield one line at a time as we print it
...
def iter_combined_plain_tuple(fnames,class_names,parsers,compress_fields):
    ...
    merged_iterator =(itertools.chain.from_iterable(zipped_tuples)for zipped_tuple in zipped_tuples)
...
def group_data(fnames,class_names,parsers,compress_fields,filter_key,group_key,gender):
    data = filtered_iter_combined(fnames,class_names,
